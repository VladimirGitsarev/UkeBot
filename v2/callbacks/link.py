...
class LinkCallback:
    ...
    @staticmethod
    def link_callback(data_value, query, links, cur_song, chords_list):
        """Generate message for songs list or a particular song"""
        if data_value in ['more', 'less', 'list']:
            return LinkCallback._link_list_callback(data_value, query, links)
        return LinkCallback._link_song_callback(data_value, cur_song, links, chords_list)

    @staticmethod
    def _link_list_callback(data_value, query, links) -> Tuple[InlineKeyboardMarkup, str]:
    ...
    @staticmethod
    def _link_song_callback(data_value, cur_song, links, chords_list) -> Tuple[InlineKeyboardMarkup, str]:
